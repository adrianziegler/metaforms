...
export async function runMigrations() {
  ...
  try {
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_email_audit_org_created ON email_audit(org_id, created_at DESC)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_email_audit_success ON email_audit(success, created_at DESC)`);
